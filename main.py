...
while not game_over:
    # loops over all of the events (like keypresses and mouse moves and button clicks) that happen during one game loop cycle
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.K_LEFT: #left arrow key
            x1_change = -10
            y1_change = 0
        elif event.type == pygame.K_RIGHT: # right arrow key
            x1_change = 10
            y1_change = 0
        elif event.type == pygame.K_UP: # up arrow key
            y1_change = -10
            x1_change = 0
        elif event.type == pygame.K_DOWN: # down arrow key
            y1_change = 10
            x1_change = 0
    ...
    y1 += y1_change
    dis.fill(white)
    #draw a yellow rectanlge to represent the head of the snake
